...
def apply_svd_quant_fusion(tensor, rank_ratio=0.5, bits=8):
    """
    Simulates storing the KV cache as two lower-rank INT8 matrices.
    Memory footprint = (Seq * Rank + Rank * Dim) * 1 byte.
    At 50% rank, this is exactly a 4x compression compared to FP16, identical to INT4.
    """
...
def custom_attention_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    ...
    if COMPRESSION_MODE == "fusion":
        # SVD 50% + INT8 (Effective 4x Compression)
        key_states = apply_svd_quant_fusion(key_states, rank_ratio=0.50, bits=8)
        value_states = apply_svd_quant_fusion(value_states, rank_ratio=0.50, bits=8)
    elif COMPRESSION_MODE == "a4kv4":
        # Pure INT4 (Effective 4x Compression)
...
def run():
    ...
    configs = [
        {"name": "1. Pure A4KV4 (Block 32) -> 4x Compression", "mode": "a4kv4"},
        {"name": "2. SVD 50% + INT8 Fusion -> 4x Compression", "mode": "fusion"}
    ]
