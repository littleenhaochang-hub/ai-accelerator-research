...
## 1. 品質評估方法論：SNR 與「dB」的意義

為了在不跑完整模型的情況下科學地衡量量化演算法的破壞力，我們採用了 **SNR (訊噪比，單位為 dB 分貝)** 作為數學指標：
*   **公式**：$10 \cdot \log_{10}\left(\frac{\text{真實 FP32 訊號變異數}}{\text{量化誤差(雜訊)變異數}}\right)$
*   **意義**：dB 值越高越好。因為它是對數尺度 (Log Scale)，**每下降 3 dB，代表「雜訊能量翻倍」**。
*   **實務閾值 (Empirical Thresholds)**：
    *   `< 10 dB`：雜訊大到模型變成聾子，輸出完全崩潰 (亂碼或沉默)。
    *   `14~15 dB`：模型還能說英文，但注意力變得「模糊」，極易產生幻覺。
    *   `> 18 dB`：量化雜訊被壓制在安全範圍內，模型內部的自迴歸邏輯 (Logic Engine) 得以保全。

---

## 2. 核心痛點：Activation Outliers 與 Softmax 懸崖
...
## 3. 常規 A4A4 解法的死胡同 (2_3_a4a4_attention_optimizations)
...
3.  **Sub-Channel Quant (分組量化, Group=32)**：把向量切成好幾塊，每塊獨立算 Scale。**代價太高**：數學上有效 (18.5 dB)，但在 KV Cache (長文本推理) 中，每 256 個 Token 就要儲存 65,536 Bytes 的 Scale factors (是常規的 128 倍)。在 Edge 設備上，這種「記憶體頻寬稅 (Memory Tax)」會直接卡死 NPU。

---

## 4. 最佳解答：TurboQuant (旋轉矩陣) + 1-Bit QJL 殘差
為了適應 Edge 設備「算力過剩、頻寬緊缺」的特性，我們確立了以 **TurboQuant** 為核心的最終架構。

*   **關鍵註解 (Symmetry in A4KV4)**：在處理全 4-bit 注意力機制時，我們對 Query ($Q$) 應用了與 $KV$ **完全相同** 的量化手法。如果 $K$ 乘上了正交矩陣 $R$ 進行旋轉，$Q$ 也必須乘上同一個矩陣 $R$ 才能讓點積空間對齊 ($Q R^T \cdot (K R^T)^T = Q K^T$)。這帶來了一個巨大的紅利：**$Q$ 裡面的離群值也順便被完美抹平了！**
...
## 5. 極限探索：1.58-bit Ternary MACs (BitNet 架構)
...
## 結論 (架構決策)
針對行動端與 Edge AI：
1.  **放棄** Attention 層的 Sub-Channel (Group) 量化與 Sparse-Dense 稀疏矩陣。
2.  **全面採用** 「TurboQuant 旋轉矩陣 (以算力換記憶體) + 1-Bit 殘差 (Popcount 硬體加速)」作為 W4A4 與 KV Cache 的標準配備。這能在擴增極小記憶體 (4-bit -> 5-bit) 的前提下，打平 FP16 的生成品質。
3.  **僅在 FFN 層** 使用 Sub-Channel 搭配 E8M0 Scale，因為 FFN 沒有 Softmax 放大器且算完即丟，不會產生記憶體頻寬稅。