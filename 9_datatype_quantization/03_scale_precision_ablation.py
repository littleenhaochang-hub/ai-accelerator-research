...
def apply_scale_format(scale, scale_format):
    if scale_format == "fp16":
        ...
        # Pure exponent: Scale is forced to be a power of 2.
        # This allows hardware to replace multipliers with bit-shifts.
        # Requires 8 bits to store the exponent.
        return 2.0 ** torch.round(torch.log2(scale + 1e-9))
    elif scale_format == "int8":
        # Double Quantization (like QLoRA): Quantize the FP16 scales into INT8.
        # Requires 8 bits per block, plus 1 global FP16 scale per tensor.
        scale_max = torch.max(scale)
        s = scale_max / 255.0
        s = torch.clamp(s, min=1e-9)
        q_scale = torch.round(scale / s)
        return q_scale * s
    return scale
...
def quantize_fp4_e2m1(tensor, block_size=32, scale_format="fp16"):
    ...
    if tensor.shape[-1] % block_size == 0:
        ...
        # Apply Hardware Scale Constraint (E8M0 or INT8 Double Quantization)
...
class ScaleAblationLinear(nn.Module):
    def __init__(self, original_linear, scale_fmt="fp16"):
        super().__init__()
    ...
    def forward(self, x):
        # We apply FP4 Block 32 to everything, only varying the SCALE precision.
...
def apply_patch(model, scale_fmt):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear) and "lm_head" not in name:
            setattr(model, name, ScaleAblationLinear(module, scale_fmt))
        else:
...
def run():
    ...
    configs = [
        {"name": "1. FP4 Block 32 (FP16 Scales) [Control: 4.50 bits/param]", "sfmt": "fp16"},
        {"name": "2. FP4 Block 32 (INT8 Scales) [Double Quant: 4.25 bits/param]", "sfmt": "int8"},
        {"name": "3. FP4 Block 32 (E8M0 Scales) [Multiplier-Free: 4.25 bits/param]", "sfmt": "e8m0"},
    ]
