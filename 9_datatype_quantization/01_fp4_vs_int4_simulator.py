...
def apply_scale_format(scale, scale_format):
    if scale_format == "e8m0":
        # Restrict scale to powers of 2 (pure exponent shift)
        return 2.0 ** torch.round(torch.log2(scale))
    return scale # fp16

def quantize_int4(tensor, block_size=None, scale_format="fp16"):
    qmin, qmax = -8, 7
...
def quantize_fp4_e2m1(tensor, block_size=None, scale_format="fp16"):
    # OCP FP4 (E2M1) positive levels
    ...
    if block_size is not None and tensor.shape[-1] % block_size == 0:
        ...
        return (torch.sign(tensor_blocked) * quantized_abs * scale).view(orig_shape)
    else:
        max_val = torch.max(torch.abs(tensor), dim=-1, keepdim=True)[0]
        scale = max_val / 6.0
        scale = torch.clamp(scale, min=1e-5)
...
class DatatypeWarLinear(nn.Module):
    def __init__(self, original_linear, dtype_mode="int4", block_size=32, scale_fmt="fp16"):
        super().__init__()
    ...
    def forward(self, x):
        if self.dtype_mode == "fp4":
            w = quantize_fp4_e2m1(self.weight, block_size=self.block_size, scale_format=self.scale_fmt)
            x_q = quantize_fp4_e2m1(x, block_size=self.block_size, scale_format=self.scale_fmt)
        else:
            w = quantize_int4(self.weight, block_size=self.block_size, scale_format=self.scale_fmt)
            x_q = quantize_int4(x, block_size=self.block_size, scale_format=self.scale_fmt)
            
        return F.linear(x_q, w, self.bias)

def apply_patch(model, dtype_mode, block_size, scale_fmt):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear) and "lm_head" not in name:
            setattr(model, name, DatatypeWarLinear(module, dtype_mode, block_size, scale_fmt))
        else:
            apply_patch(module, dtype_mode, block_size, scale_fmt)
...
def run():
    ...
    configs = [
        {"name": "1. INT4 (Block 32, FP16 Scale) [Control]", "dtype": "int4", "blk": 32, "sfmt": "fp16"},
        {"name": "2. INT4 (Block 32, E8M0 Scale) [Cheap Scale]", "dtype": "int4", "blk": 32, "sfmt": "e8m0"},
        {"name": "3. FP4  (Block 32, FP16 Scale) [OCP Target]", "dtype": "fp4", "blk": 32, "sfmt": "fp16"},
        {"name": "4. FP4  (Block 32, E8M0 Scale) [Ultimate Hardware]", "dtype": "fp4", "blk": 32, "sfmt": "e8m0"},
        {"name": "5. FP4  (No Block 32, FP16 Scale) [Stress Test]", "dtype": "fp4", "blk": None, "sfmt": "fp16"},
    ]
    ...
    for cfg in configs:
        ...
        apply_patch(model, dtype_mode=cfg["dtype"], block_size=cfg["blk"], scale_fmt=cfg["sfmt"])
