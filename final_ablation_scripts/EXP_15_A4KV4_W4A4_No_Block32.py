...
# --- W4A4 FFN (NO Block 32! Pure Uniform Quantization!) ---
class W4A4_Uniform_Linear(nn.Module):
    ...
    def forward(self, x):
        # NO block_size=32! This is the naive uniform quantization (per-channel or per-tensor scale)
        w = fake_quantize(self.weight, bits=4, block_size=None)
        x_q = fake_quantize(x, bits=4, block_size=None)
...
def apply_full_mixed_patch(model):
    for i in range(1, NUM_LAYERS - 1): # Mid layers only (1 to 22)
        ...
        # Patch FFN with Uniform W4A4 (No Block32)
        layer.mlp.gate_proj = W4A4_Uniform_Linear(layer.mlp.gate_proj)
        layer.mlp.up_proj = W4A4_Uniform_Linear(layer.mlp.up_proj)
        layer.mlp.down_proj = W4A4_Uniform_Linear(layer.mlp.down_proj)
        
        # Patch Attention Projections with Uniform W4A4 (No Block32)
        layer.self_attn.q_proj = W4A4_Uniform_Linear(layer.self_attn.q_proj)
        layer.self_attn.k_proj = W4A4_Uniform_Linear(layer.self_attn.k_proj)
        layer.self_attn.v_proj = W4A4_Uniform_Linear(layer.self_attn.v_proj)
        layer.self_attn.o_proj = W4A4_Uniform_Linear(layer.self_attn.o_proj)
...
def run():
    ...
    print(f"\n======================================")
    print(f"Testing: EXP 15 (A4KV4 + W4A4 ALL UNIFORM - NO BLOCK 32)")
