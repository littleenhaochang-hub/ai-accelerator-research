...
# --- 1D Feature Hadamard Attention (A4KV4) ---
...
# --- SmoothQuant W4A4 Linear ---
class SmoothQuantLinear(nn.Module):
    def __init__(self, original_linear, alpha=0.5):
        super().__init__()
    ...
    def forward(self, x):
        # Dynamic SmoothQuant formulation: 
        # Calculate per-channel max of activations and weights
        # We do this dynamically here to simulate the absolute BEST possible PTQ calibration (Oracle bounds)
        
        # x shape: (batch, seq, in_features)
        # w shape: (out_features, in_features)
        
        # Flatten batch and seq to find max magnitude per input feature channel
        x_flat = x.view(-1, self.in_features)
        act_max = torch.max(torch.abs(x_flat), dim=0)[0]
        act_max = torch.clamp(act_max, min=1e-5)
        
        weight_max = torch.max(torch.abs(self.weight), dim=0)[0]
        weight_max = torch.clamp(weight_max, min=1e-5)
        
        # Calculate smoothing scale (s)
        # s = (max|X|^alpha) / (max|W|^(1-alpha))
        scale = (act_max.pow(self.alpha) / weight_max.pow(1.0 - self.alpha)).clamp(min=1e-5)
        
        # Apply smoothing
        # X' = X / diag(s) => divide along the last dimension
        x_smoothed = x / scale
        
        # W' = W * diag(s) => multiply along the input features dimension
        w_smoothed = self.weight * scale
        
        # Now apply the hardware 4-bit quantization (Block 32)
        x_q = fake_quantize(x_smoothed, bits=4, block_size=32)
        w_q = fake_quantize(w_smoothed, bits=4, block_size=32)
        
        # Matrix multiplication on smoothed and quantized tensors
        return F.linear(x_q, w_q, self.bias)

def apply_smoothquant_patch(model, alpha=0.5):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear):
            if "lm_head" not in name:
                setattr(model, name, SmoothQuantLinear(module, alpha))
        else:
            apply_smoothquant_patch(module, alpha)
...
def run():
    ...
    configs = [
        {"name": "1. SmoothQuant α=0.5 (A4KV4 + W4A4 FFN)", "alpha": 0.5},
        {"name": "2. SmoothQuant α=0.75 (Heavier Activation Smoothing)", "alpha": 0.75},
    ]
    ...
    for cfg in configs:
        ...
        # Apply SmoothQuant and A4KV4 Patches
        apply_smoothquant_patch(model, alpha=cfg['alpha'])
