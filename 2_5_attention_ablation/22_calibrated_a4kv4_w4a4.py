...
# --- Dynamic Calibrator (Hardware Simulation) ---
def calibrate_activation(tensor, method="std", factor=3.5):
    """
    Simulates a hardware calibration logic module placed before the FFN ALU.
    It dampens the cascading quantization noise from the KV Cache by clamping
    spurious outlier spikes before they ruin the Block 32 scaling factors.
    """
    if method == "std":
        # Dynamic thresholding based on standard deviation
        mu = tensor.mean(dim=-1, keepdim=True)
        sigma = tensor.std(dim=-1, keepdim=True)
        threshold = factor * sigma
        # Clamp outliers
        return torch.clamp(tensor, mu - threshold, mu + threshold)
    elif method == "rms":
        # RMS based clipping
        rms = torch.sqrt(torch.mean(tensor**2, dim=-1, keepdim=True))
        return torch.clamp(tensor, -factor * rms, factor * rms)
    return tensor
...
# --- FFN Calibrated Linear ---
class FFN_Calibrated_Linear(nn.Module):
    def __init__(self, original_linear, act_bits, apply_calib=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.apply_calib:
            # Calibrate out the cascading noise from KV4
            x = calibrate_activation(x, method="std", factor=3.5)
            
        x_q = fake_quantize(x, bits=self.act_bits, block_size=32)
        return F.linear(x_q, w, self.bias)

def apply_patch(model, act_bits, apply_calib):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear):
            if "lm_head" not in name:
                setattr(model, name, FFN_Calibrated_Linear(module, act_bits, apply_calib))
        else:
            apply_patch(module, act_bits, apply_calib)
...
def run():
    ...
    configs = [
        {"name": "1. Uncalibrated A4KV4 + A8W4 FFN (Baseline failure)", "act_bits": 8, "calib": False},
        {"name": "2. Calibrated A4KV4 + A8W4 FFN (Clamping Noise)", "act_bits": 8, "calib": True},
        {"name": "3. Calibrated A4KV4 + A4W4 FFN (The Holy Grail)", "act_bits": 4, "calib": True},
    ]
    ...
    for cfg in configs:
        ...
        apply_patch(model, act_bits=cfg['act_bits'], apply_calib=cfg['calib'])
