...
# We use a global config to toggle A8 vs A16
ATTN_ACT_BITS = 16 
...
def custom_attention_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    # Quantize input activations to Attention if A8
    if ATTN_ACT_BITS == 8:
    ...
    # 2D Hadamard Transform (Token + Feature)
    ...
    # Token Dimension Hadamard
    seq_len = q_len
    if seq_len > 1 and seq_len % 2 == 0:
        if seq_len not in HADAMARD_MATRICES:
            HADAMARD_MATRICES[seq_len] = get_hadamard_matrix(seq_len, query_states.device, query_states.dtype)
        H_tok = HADAMARD_MATRICES[seq_len]
        
        # apply Token Hadamard on KV (not Q for decode usually, but for prefill we can)
        key_states = H_tok @ key_states
        value_states = H_tok @ value_states

    # Feature Dimension Hadamard
    query_states = query_states @ H_feat
    key_states = key_states @ H_feat
    value_states = value_states @ H_feat

    # KV Cache is always 4-bit (KV4)
    ...
    if seq_len > 1 and seq_len % 2 == 0:
        attn_output = H_tok.T @ attn_output

    attn_output = attn_output.transpose(1, 2).contiguous()
    attn_output = attn_output.reshape(bsz, q_len, self.config.hidden_size)
    
    # Quantize input to Output Projection if A8
    if ATTN_ACT_BITS == 8:
...
def run():
    ...
    # Get Baseline FP16 Hidden States
    ...
    configs = [
        {"name": "Attention A16KV4 + FFN A4W4", "attn_bits": 16},
        {"name": "Attention A8KV4 + FFN A4W4", "attn_bits": 8},
    ]

    for cfg in configs:
        print(f"\nRunning Config: {cfg['name']}")
        ATTN_ACT_BITS = cfg['attn_bits']
