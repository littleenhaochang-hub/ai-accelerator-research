...
# --- A8KV8 Attention Block ---
def a8kv8_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    ...
    # NO Hadamard for A8KV8 (8-bit has enough dynamic range)
    key_states = fake_quantize(key_states, bits=8)
    value_states = fake_quantize(value_states, bits=8)
...
def mixed_attention_wrapper(self, *args, **kwargs):
    ...
    if layer_idx == 0 or layer_idx == NUM_LAYERS - 1:
        ...
        return a8kv8_forward(self, *args, **kwargs)
...
def run():
    ...
    print(f"\n======================================")
    print(f"Testing: Mixed-Precision Fusion (A8KV8 Attention + FFN W4A4 Block 32)")
