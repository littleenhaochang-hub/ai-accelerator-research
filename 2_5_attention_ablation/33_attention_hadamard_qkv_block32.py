...
# --- EXP 17: 1D Hadamard + Q/K/V Block 32 Quantization ---
def hadamard_qkv_block32_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    # A4 input with block 32
    hidden_states = fake_quantize(hidden_states, bits=4, block_size=32)
    ...
    # 1D Feature Hadamard Smearing
    query_states = query_states @ H_feat
    key_states = key_states @ H_feat
    value_states = value_states @ H_feat

    # **NEW**: Q, K, and V all subjected to A4 Block 32 quantization after Hadamard!
    query_states = fake_quantize(query_states, bits=4, block_size=32)
    key_states = fake_quantize(key_states, bits=4, block_size=32)
    value_states = fake_quantize(value_states, bits=4, block_size=32)
    ...
    # A4 output with block 32
    attn_output = fake_quantize(attn_output, bits=4, block_size=32)
...
def mixed_attention_wrapper(self, *args, **kwargs):
    ...
    if layer_idx == 0 or layer_idx == NUM_LAYERS - 1:
        ...
        return hadamard_qkv_block32_forward(self, *args, **kwargs)
...
def apply_full_mixed_patch(model):
    for i in range(1, NUM_LAYERS - 1): # Mid layers only (1 to 22)
        ...
        layer.self_attn.q_proj = W4A4_Block32_Linear(layer.self_attn.q_proj)
        layer.self_attn.k_proj = W4A4_Block32_Linear(layer.self_attn.k_proj)
        layer.self_attn.v_proj = W4A4_Block32_Linear(layer.self_attn.v_proj)
        layer.self_attn.o_proj = W4A4_Block32_Linear(layer.self_attn.o_proj)
...
def run():
    ...
    print(f"\n======================================")
    print(f"Testing: EXP 17 (1D Hadamard + Q/K/V Block 32 + W4A4 Proj + W4A4 FFN)")
