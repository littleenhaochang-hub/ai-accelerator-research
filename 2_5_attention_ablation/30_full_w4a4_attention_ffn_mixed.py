...
def a4kv4_1d_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    # Hidden states is quantized to A4 (block32) inside the q/k/v proj already because they are W4A4_Block32_Linear
    # But just in case, we follow the pipeline
    ...
    # Send through o_proj (which is also W4A4_Block32_Linear, so it will quantize input to A4 block32 natively)
...
def apply_full_mixed_patch(model):
    for i in range(1, NUM_LAYERS - 1): # Mid layers only (1 to 22)
        ...
        # Patch Attention Projections (W4A4 Block 32)
...
def run():
    ...
    print(f"\n======================================")
    print(f"Testing: Mixed-Precision Fusion (Attention W4A4 + A4KV4) & (FFN W4A4 Block 32)")
