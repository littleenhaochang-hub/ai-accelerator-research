...
# --- Route C: A8 Attention + KV8 + FFN W4A4 ---
def a8kv8_attention_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    # A8 Input to Attention
    ...
    # KV8 Cache (No Hadamard, just straight 8-bit which has 256 levels, usually enough for outliers)
...
def run():
    ...
    for i, q_data in enumerate(QUESTIONS):
        ...
        if is_pass: passed += 1
        # print(f"Q{i+1}: {'PASS' if is_pass else 'FAIL'} | {ans[:50]}...")
        
    print(f"-> Final Pass Rate: {(passed/20)*100:.1f}% ({passed}/20)")
    
    # Compute Baseline Metric (Cosine Sim) for one sequence
