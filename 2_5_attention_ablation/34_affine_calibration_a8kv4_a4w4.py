...
# Global variables for calibration stats
CALIBRATION_MODE = False
global_stats = {}

# --- A8KV4 1D Hadamard Forward with Inter-Layer Affine Calibration ---
def a8kv4_1d_calibrated_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, past_key_values=None, output_attentions=False, use_cache=False, cache_position=None, position_embeddings=None, **kwargs):
    # A8 input
    ...
    # --- AFFINE CALIBRATION BLOCK (The "Noise Scrubber") ---
    # This simulates a hardware block that re-centers the distribution 
    # based on offline calibration statistics before it hits the FFN A4 quantizer.
    layer_name = f"layer_{self.layer_idx}"
    if CALIBRATION_MODE:
        # Collect stats during offline pass
        if layer_name not in global_stats:
            global_stats[layer_name] = {'mean': 0.0, 'std': 0.0, 'count': 0}
        
        # EMA update
        current_mean = attn_output.mean(dim=(0,1)).detach()
        current_std = attn_output.std(dim=(0,1)).detach()
        
        c = global_stats[layer_name]['count']
        global_stats[layer_name]['mean'] = (global_stats[layer_name]['mean'] * c + current_mean) / (c + 1)
        global_stats[layer_name]['std'] = (global_stats[layer_name]['std'] * c + current_std) / (c + 1)
        global_stats[layer_name]['count'] += 1
    else:
        # Apply inference correction (Scale & Shift)
        if layer_name in global_stats:
            # We apply a slight restorative shift based on the difference between expected vs corrupted output.
            # In a true PTQ setup, we'd compare FP16 stats vs A8KV4 stats. Here, we simulate the "Shift" 
            # by pushing the tensor back towards a normalized Gaussian before the destructive A4 FFN step.
            mean_shift = global_stats[layer_name]['mean']
            std_scale = global_stats[layer_name]['std'] + 1e-5
            
            # Re-center and gently scale to prevent outlier blowout before FFN W4A4
            attn_output = (attn_output - mean_shift) / std_scale
            # Restore to original magnitude range roughly, but centered
            attn_output = attn_output * std_scale.mean() + mean_shift.mean()
...
def mixed_attention_wrapper(self, *args, **kwargs):
    layer_idx = self.layer_idx
    if layer_idx == 0 or layer_idx == NUM_LAYERS - 1:
        return ORIGINAL_FORWARD(self, *args, **kwargs)
    else:
        return a8kv4_1d_calibrated_forward(self, *args, **kwargs)

def apply_full_mixed_patch(model):
    for i in range(1, NUM_LAYERS - 1): # Mid layers only (1 to 22)
        layer = model.model.layers[i]
        layer.mlp.gate_proj = W4A4_Block32_Linear(layer.mlp.gate_proj)
        layer.mlp.up_proj = W4A4_Block32_Linear(layer.mlp.up_proj)
        layer.mlp.down_proj = W4A4_Block32_Linear(layer.mlp.down_proj)
...
def run():
    ...
    apply_full_mixed_patch(model)
    Qwen2Attention.forward = mixed_attention_wrapper
    
    print("\n--- Phase 1: Offline Calibration (Running calibration text) ---")
    CALIBRATION_MODE = True
    calib_texts = [
        "The quick brown fox jumps over the lazy dog.",
        "In mathematics, an integral assigns numbers to functions in a way that describes displacement, area, volume, and other concepts.",
        "To cook a perfect steak, ensure the pan is smoking hot before adding the oil and butter."
    ]
    for t in calib_texts:
        inp = tokenizer([tokenizer.apply_chat_template([{"role": "user", "content": t}], tokenize=False, add_generation_prompt=True)], return_tensors="pt").to(model.device)
        with torch.no_grad():
            model(**inp)
    
    print("Calibration stats collected for all layers.")

    print(f"\n--- Phase 2: Inference with Scale & Shift Block ---")
    print(f"Testing: A8KV4 Attention + FFN W4A4 Block32 + Affine Calibration")
    CALIBRATION_MODE = False
