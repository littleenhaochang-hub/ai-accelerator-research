...
def run_macro_qat():
    ...
    print(f"Loading {model_id} for MACRO QAT (W1.58A16)...")
    ...
    model.train()
    print(f"Starting Macro QAT Loop ({steps} steps, CosineAnnealingLR, Peak LR={lr})...")
    ...
    for step in range(steps):
        ...
        if (step + 1) % 2500 == 0:
            checkpoint_dir = f"ai-accelerator-research/checkpoints/gemma_w158_step_{step+1}"
            os.makedirs(checkpoint_dir, exist_ok=True)
            print(f"--> Reached step {step+1}, checkpointing logic active (weights in VRAM).")

    print("\nMacro QAT Training Complete. Generating Final Weights...")
    # Trigger final evaluation script logic later
